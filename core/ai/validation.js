...
function sanitizePayload(value) {
  ...
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).slice(0, 80).map(([key, item]) => [key, sanitizePayload(item)]));
  return value;
}
...
function validateCommon(review, context, errors) {
  ...
  for (const key of collectKeys(review)) if (FORBIDDEN_KEYS.has(key)) errors.push(`AI output attempted to mutate deterministic field: ${key}`);
...
function validateAISummary(rawSummary, context = {}) {
  ...
  for (const key of collectKeys(summary)) if (FORBIDDEN_KEYS.has(key)) errors.push(`AI summary attempted to mutate deterministic field: ${key}`);
